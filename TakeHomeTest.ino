...
// for SPI-mode, make sure that R1 is populated with a 470k resistor (or whichever resistor connects pins 24 & 25 of the
// 1CA027 IC). The pad is configured for Absolute mode tracking.  Touch data is sent in text format over USB CDC to the
// host PC.  You can open a terminal window on the PC to the USB CDC port and see X, Y, and Z data fill the window when
// you touch the sensor. Tools->Serial Monitor can be used to view touch data. NOTE: all config values applied in this
// sample are meant for a module using REXT = 976kOhm
...
const uint8_t ZVALUE_MAP[ROWS_Y][COLS_X] = {
    {0, 0, 0, 0, 0, 0, 0, 0},   {0, 2, 3, 5, 5, 3, 2, 0}, {0, 3, 5, 15, 15, 5, 2, 0},
    {0, 3, 5, 15, 15, 5, 3, 0}, {0, 2, 3, 5, 5, 3, 2, 0}, {0, 0, 0, 0, 0, 0, 0, 0},
};
...
// User code
typedef enum { Q0, Q1, Q2, Q3 } quadrant_e;
typedef enum { NO_TOUCH, TOUCHING, REPORT } fsm_e;
...
bool Pinnacle_zIdlePacket(absData_t* data) { return data->xValue == 0 && data->yValue == 0 && data->zValue == 0; }
...
// Explanation: Consider the response of the sensor to be flat across it's area. The Z-sensitivity of the sensor
// projects this area a short distance upwards above the surface of the sensor. Imagine it is a solid cylinder (wider
// than it is tall) in which a finger can be detected and tracked. Adding a curved overlay will cause a user's finger to
// dip deeper in the middle, and higher on the perimeter. If the sensitivity is tuned such that the sensing area
// projects to the highest part of the overlay, the lowest point will likely have excessive sensitivity. This means the
// sensor can detect a finger that isn't actually contacting the overlay in the shallower area. ZVALUE_MAP[][] stores a
// lookup table in which you can define the Z-value and XY position that is considered "hovering".
// Experimentation/tuning is required. NOTE: Z-value output decreases to 0 as you move your finger away from the sensor,
// and it's maximum value is 0x63 (6-bits).
...
void AssertSensorLED(bool state) { digitalWrite(LED_0, !state); }
